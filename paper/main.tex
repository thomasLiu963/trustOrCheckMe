...
% Review mode hardcodes Affiliation/Address/email in neurips_2026.sty.
% Override the visible title-block extras without editing the style file.
\makeatletter
\patchcmd{\@maketitle}{%
        Anonymous Author(s) \\
        Affiliation \\
        ...
        Anonymous Author(s) \\
}{}{\errmessage{Failed to strip anonymous affiliation placeholders}}
\makeatother
...
Prompt robustness uses the fixed 100-question subset with the same frozen answers and reported confidences under one semantically equivalent Stage~3 paraphrase. The prespecified robustness analysis covered the hidden-confidence condition (3,200 decisions). After completion of the primary experiment, we ran the corresponding visible-confidence extension (3,200 additional decisions). We therefore distinguish the hidden-condition robustness analysis as prespecified in the frozen protocol and the visible-condition extension as post-primary. The frozen protocol designated decision authority as the primary question and confidence visibility as a mechanism question; Appendix~A documents this prespecified framing and the manuscript's subsequent change in emphasis.
